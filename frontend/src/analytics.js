...
import { faChevronLeft, faPause, faPlay, faDumbbell, faHeartbeat, faBolt, faUtensils, faCoffee, faRunning, faHome, faGlobeAmericas, faChartBar, faUser, faUpload, faSpinner } from '@fortawesome/free-solid-svg-icons';
...
const UploadView = ({ onUploadSuccess, setLoading }) => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            alert('An error occurred while uploading the file: ' + (error.message || 'Unknown error'));
        } finally {
            setUploading(false);
            setUploadProgress(0);
        }
...
const WorkoutApp = () => {
    ...
    return (
        <div className="app-container relative pb-20">
            <AnimatePresence mode="wait">
                {view === 'upload' && (
                    <UploadView onUploadSuccess={handleUploadSuccess} setLoading={setLoading} />
                )}
                {view === 'exercises' && (
                    <ExercisesView
